import React, { useMemo } from 'react';
import { View, Text, TouchableOpacity, ScrollView } from 'react-native';
import { MaterialIcons } from '@expo/vector-icons';
import tw from '../styles';
import gameStore from '../gameEngine';

const SPOT_GOODS = [
  { id: 'egg', icon: '🥚', name: 'Raw Eggs', price: 2.2 },
  { id: 'freeRange', icon: '🐓', name: 'Free Range', price: 5.5 },
  { id: 'organic', icon: '🌿', name: 'Organic', price: 7.8 },
  { id: 'powdered', icon: '🥛', name: 'Powdered', price: 6.0 },
  { id: 'mayo', icon: '🫙', name: 'Mayo Jar', price: 14 },
  { id: 'omelette', icon: '🍳', name: 'Omelette', price: 11.5 },
  { id: 'cake', icon: '🎂', name: 'Egg Cake', price: 37 },
  { id: 'custard', icon: '🍮', name: 'Custard', price: 18 },
  { id: 'vaccine', icon: '💉', name: 'Protein Serum', price: 200 },
];

export default function MarketZone({ gameState }) {
  const mult = gameStore.getPriceMultiplier();

  // Stable ticker prices — only recomputed when mult changes, not on every render
  const tickerPrices = useMemo(() => (
    SPOT_GOODS.map(g => ({
      ...g,
      displayPrice: (g.price * mult).toFixed(2),
      up: Math.random() > 0.5,
    }))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ), [Math.round(mult * 10)]); // recompute only when trend changes materially

  const activeContracts = gameState.contracts.filter(c => !c.accepted);

  return (
    <ScrollView contentContainerStyle={tw`pb-32 px-4 bg-slate-100`}>

      {/* ── Market Ticker ── */}
      <View style={tw`bg-slate-900 rounded-2xl overflow-hidden shadow-lg border border-slate-700 p-3 mb-6`}>
        <View style={tw`flex-row justify-between items-center mb-2`}>
          <Text style={tw`text-[9px] font-interBlack text-slate-500 uppercase tracking-wider`}>Live Market Rates</Text>
          <View style={tw`flex-row items-center gap-2`}>
            <Text style={tw`text-[9px] font-black uppercase ${mult > 1 ? 'text-green-500' : 'text-red-500'}`}>
              Trend: {(mult * 100).toFixed(0)}%
            </Text>
            {gameState.frenzyActive > 0 && (
              <Text style={tw`text-[9px] font-black bg-primary text-white px-1.5 rounded`}>FRENZY!</Text>
            )}
            {gameState.festivalActive > 0 && (
              <Text style={tw`text-[9px] font-black bg-purple-500 text-white px-1.5 rounded`}>FESTIVAL!</Text>
            )}
          </View>
        </View>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={tw`flex-row items-center mt-2`}>
          {tickerPrices.map((g, i) => (
            <View key={i} style={tw`flex-row items-center gap-1.5 mx-4`}>
              <Text style={tw`text-xs font-bold text-slate-400`}>{g.name}</Text>
              <Text style={tw`text-xs font-black ${g.up ? 'text-green-500' : 'text-red-500'}`}>
                {g.up ? '▲' : '▼'} ${g.displayPrice}
              </Text>
            </View>
          ))}
        </ScrollView>
      </View>

      {/* ── Rush Mode Banner ── */}
      {gameState.rushMode && (
        <View style={tw`bg-amber-50 border border-amber-300 rounded-xl px-4 py-3 mb-4 flex-row items-center justify-between`}>
          <Text style={tw`text-[11px] font-black text-amber-800`}>🔥 SUPPLY RUSH ACTIVE — {gameState.rushTimer}s left</Text>
          <Text style={tw`text-[10px] font-bold text-amber-600`}>+50% earnings!</Text>
        </View>
      )}

      {/* ── Boost / Festival Banners ── */}
      {gameState.spinProdBoost > 0 && (
        <View style={tw`bg-blue-50 border border-blue-200 rounded-xl px-4 py-2 mb-3`}>
          <Text style={tw`text-xs font-bold text-blue-700`}>⚡ 2x Production active: {gameState.spinProdBoost}s remaining</Text>
        </View>
      )}
      {gameState.festivalActive > 0 && (
        <View style={tw`bg-purple-50 border border-purple-200 rounded-xl px-4 py-2 mb-3`}>
          <Text style={tw`text-xs font-bold text-purple-700`}>🎪 Market Festival! Prices 2.5x for {gameState.festivalActive}s</Text>
        </View>
      )}

      {/* ── Contracts ── */}
      <View style={tw`mb-6`}>
        <View style={tw`flex-row justify-between items-center mb-3 px-1`}>
          <Text style={tw`text-xs font-interBlack text-slate-800 uppercase tracking-wider`}>Active Contracts</Text>
          <View style={tw`flex-row items-center gap-2`}>
            <TouchableOpacity
              onPress={() => gameStore.refreshContracts()}
              style={tw`px-3 py-1.5 bg-slate-100 border border-slate-300 rounded-lg flex-row items-center gap-1`}
            >
              <Text style={tw`text-[9px] font-black uppercase text-slate-600`}>🔁 Refresh</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => gameStore.startSupplyRush()}
              style={tw`px-3 py-1.5 bg-amber-100 border border-amber-300 rounded-lg`}
            >
              <Text style={tw`text-[9px] font-black uppercase text-amber-800`}>⚡ Rush</Text>
            </TouchableOpacity>
          </View>
        </View>

        {activeContracts.length === 0 ? (
          <View style={tw`bg-white rounded-2xl p-6 items-center shadow-sm border border-slate-200`}>
            <Text style={tw`text-xs font-bold text-slate-400`}>No active contracts. Tap refresh above.</Text>
          </View>
        ) : (
          <View style={tw`flex-col gap-3`}>
            {activeContracts.map(c => {
              const have = Math.floor(gameState.inv[c.want] || 0);
              const canFill = have >= c.qty;
              const earn = Math.floor(c.qty * c.pricePerUnit * mult);
              const timeLeft = Math.floor(c.ticksLeft / 60);

              return (
                <View key={c.id} style={tw`bg-white rounded-2xl p-4 shadow-sm border ${c.urgent ? 'border-amber-400' : 'border-slate-200'} flex-row items-start gap-4`}>
                  <View style={tw`w-12 h-12 rounded-full bg-slate-50 items-center justify-center border border-slate-100`}>
                    <MaterialIcons name="business" size={24} color="#94a3b8" />
                  </View>
                  <View style={tw`flex-1`}>
                    <Text style={tw`text-sm font-interBlack text-slate-800`}>{c.buyer.split(' ').slice(1).join(' ')}</Text>
                    <Text style={tw`text-[10px] text-slate-500 font-interBold uppercase tracking-wider mb-2`}>
                      Needs: {c.qty} {c.want} · Have: {have}
                    </Text>
                    <View style={tw`flex-row flex-wrap gap-2 mb-3`}>
                      <View style={tw`bg-green-50 px-2 py-0.5 rounded-full`}>
                        <Text style={tw`text-[9px] font-interBlack text-green-600 uppercase`}>Pay: ${earn}</Text>
                      </View>
                      <View style={tw`bg-slate-100 px-2 py-0.5 rounded-full`}>
                        <Text style={tw`text-[9px] font-black text-slate-500 uppercase`}>{timeLeft}m left</Text>
                      </View>
                      {c.urgent && (
                        <View style={tw`bg-amber-50 px-2 py-0.5 rounded-full`}>
                          <Text style={tw`text-[9px] font-black text-amber-600 uppercase`}>Urgent</Text>
                        </View>
                      )}
                    </View>
                    <TouchableOpacity
                      disabled={!canFill}
                      onPress={() => gameStore.acceptContract(c.id)}
                      style={tw`w-full py-2.5 ${canFill ? 'bg-green-500 shadow-md' : 'bg-slate-100'} rounded-xl items-center active:scale-95`}
                    >
                      <Text style={tw`text-xs font-black uppercase ${canFill ? 'text-white' : 'text-slate-400'}`}>
                        {canFill ? 'Fulfill Contract ✓' : `Need ${c.qty - have} more`}
                      </Text>
                    </TouchableOpacity>
                  </View>
                </View>
              );
            })}
          </View>
        )}
      </View>

      {/* ── Spot Market ── */}
      <View style={tw`mb-6`}>
        <Text style={tw`text-xs font-black text-slate-800 uppercase tracking-wider mb-3 px-1`}>Spot Market</Text>
        <View style={tw`flex-row flex-wrap gap-3 justify-between`}>
          {SPOT_GOODS.map(g => {
            const avail = Math.floor(gameState.inv[g.id] || 0);
            const price = (g.price * mult).toFixed(2);

            return (
              <View key={g.id} style={tw`w-[48%] bg-white rounded-3xl p-4 shadow-sm border border-slate-200 items-center`}>
                <View style={tw`relative w-14 h-14 rounded-2xl bg-slate-50 items-center justify-center mb-3 shadow-inner border border-slate-100`}>
                  <Text style={tw`text-3xl`}>{g.icon}</Text>
                  {avail > 0 && (
                    <View style={tw`absolute -top-1 -right-1 bg-primary px-1.5 py-0.5 rounded-full border border-white`}>
                      <Text style={tw`text-[8px] font-black text-white`}>{avail}</Text>
                    </View>
                  )}
                </View>
                <Text style={tw`text-xs font-black text-slate-800 text-center mb-1`}>{g.name}</Text>
                <Text style={tw`text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-3`}>${price} / unit</Text>

                <View style={tw`flex-col gap-2 w-full`}>
                  <TouchableOpacity
                    disabled={avail < 10}
                    style={tw`w-full py-1.5 items-center justify-center rounded-lg ${avail < 10 ? 'bg-slate-50' : 'bg-slate-100'}`}
                    onPress={() => gameStore.sellSpot(g.id, 10)}
                  >
                    <Text style={tw`font-black text-[9px] uppercase text-slate-500`}>Sell 10</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    disabled={avail < 1}
                    style={tw`w-full py-2 items-center justify-center rounded-lg ${avail >= 1 ? 'bg-primary/10 border border-primary/20' : 'bg-slate-50'}`}
                    onPress={() => gameStore.sellSpot(g.id, avail)}
                  >
                    <Text style={tw`font-black text-[10px] uppercase ${avail >= 1 ? 'text-primary' : 'text-slate-400'}`}>
                      Sell All {avail > 0 ? `(${avail})` : ''}
                    </Text>
                  </TouchableOpacity>
                </View>
              </View>
            );
          })}
        </View>
      </View>

    </ScrollView>
  );
}