...
export default function FactoryZone({ gameState }) {
    ...
    return (
      ...
      {/* Inventory */}
      <View style={tw`mb-6`}>
        <Text style={tw`text-xs font-interBlack text-slate-800 uppercase tracking-wider mb-3 px-1`}>Your Stock</Text>
        {allGoods.length === 0 ? (
          ...
          <View style={tw`flex-row flex-wrap gap-2`}>
              {allGoods.map(([k, v]) => (
                  <View key={k} style={tw`bg-white border border-slate-200 shadow-sm rounded-lg px-3 py-1.5 flex-row items-center gap-2`}>
                      <MaterialIcons name="inventory-2" size={12} color="#94a3b8" />
                      <Text style={tw`text-[9px] font-interBold text-slate-400 uppercase tracking-wider`}>{k}</Text>
                      <Text style={tw`text-xs font-interBlack text-primary`}>{v < 10 ? v.toFixed(1) : Math.floor(v)}</Text>
                  </View>
              ))}
          </View>
        )}
                ...
                {gameState.factoryQueue.map((j, i) => {
                    ...
                    return (
                        <View key={j.id} style={tw`bg-white rounded-xl p-3 shadow-sm border ${active ? 'border-primary/50' : 'border-slate-200'} flex-row items-center gap-3`}>
                           <View style={tw`w-10 h-10 rounded-lg bg-slate-50 items-center justify-center`}>
                              <Text style={tw`text-xl`}>{j.icon}</Text>
                           </View>
                           <View style={tw`flex-1`}>
                              <Text style={tw`text-[11px] font-interBold text-slate-800 mb-1.5`}>{j.name}</Text>
      ...
      {/* Recipes */}
      <View style={tw`mb-6`}>
        <Text style={tw`text-xs font-black text-slate-800 uppercase tracking-wider mb-3 px-1`}>
                ...
                return (
                    <View key={r.id} style={tw`bg-white rounded-2xl p-4 shadow-sm border border-slate-200 flex-col gap-3`}>
                        <View style={tw`flex-row items-start gap-4`}>
                            <View style={tw`w-14 h-14 rounded-xl bg-slate-50 items-center justify-center flex-shrink-0 border border-slate-100 shadow-inner`}>
                                <Text style={tw`text-3xl`}>{r.icon}</Text>
                            </View>
                            <View style={tw`flex-1`}>
                                <Text style={tw`text-sm font-black text-slate-800 mb-0.5`}>{r.name}</Text>
                            ...
                            <TouchableOpacity disabled={!canMake} onPress={() => gameStore.queueRecipe(r.id)} style={tw`px-4 py-2 ${canMake ? 'bg-primary' : 'bg-slate-100'} rounded-lg items-center shadow-sm active:scale-95`}>
                                <Text style={tw`text-[10px] font-black uppercase ${canMake ? 'text-white' : 'text-slate-400'}`}>Queue</Text>
                            </TouchableOpacity>
                        </View>

                        <View style={tw`pt-3 border-t border-slate-100 flex-row flex-wrap gap-1.5`}>
                            {Object.entries(r.inputs).map(([k, v]) => {
                                const have = Math.floor(gameState.inv[k] || 0);
                                const ok = have >= v;
                                return (
                                    <View key={k} style={tw`px-2 py-0.5 rounded-md ${ok ? 'bg-green-50' : 'bg-red-50'} flex-row items-center gap-1`}>
                                        <MaterialIcons name={ok ? 'check' : 'close'} size={8} color={ok ? '#15803d' : '#dc2626'} />
